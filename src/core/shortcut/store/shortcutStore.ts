...
export class ShortcutManager {
  private extensions: ShortcutExtension[] = [];
   ...
   * @param extension extension phím tắt cần đăng ký
   */
  registerPlugin(extension: ShortcutExtension) {
  ...
  unregisterPlugin(name: string) {
    this.extensions = this.extensions.filter((p) => p.name !== name);
  }
  ...
  /**
   * Lấy danh sách các phím tắt hiện tại cùng action và extension tương ứng.
   * @returns Mảng các đối tượng chứa hotkey, action và extension
   */
  getShortcuts(): Array<{ hotkey: string; action: string; extension: string }> {
    return Object.entries(this.config).map(([hotkey, actionName]) => {
      const extension = this.extensions.find((p) => p.actions[actionName]);
      return {
        hotkey,
        action: actionName,
        extension: extension?.name || "unknown"
  ...
  /**
   * Lấy danh sách tất cả các action có sẵn từ các extension đã đăng ký.
   * @returns Mảng các đối tượng chứa extension và action
   */
  getAvailableActions(): Array<{ extension: string; action: string }> {
