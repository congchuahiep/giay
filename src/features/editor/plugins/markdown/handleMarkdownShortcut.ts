import { MARKDOWN_SHORTCUT_MAP } from "@/features/editor/plugins/markdown";
...
export default function handleMarkdownShortcut(
  ...
  const shortcut = Object.keys(MARKDOWN_SHORTCUT_MAP).find(
    (key) => text === key
  );
  ...
  Transforms.setNodes(
    editor,
    { type: MARKDOWN_SHORTCUT_MAP[shortcut] },
    {
      match: (n) => Element.isElement(n) && Editor.isBlock(editor, n),
    }
  );
