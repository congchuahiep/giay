 ...
 * @param targetBlockType Loại khối cần chèn (mặc định là "paragraph").
 * @returns {boolean} Trả về false nếu không thể chèn, ngược lại không trả về gì.
 */
export default function insertBreak(
  editor: Editor,
  targetBlockType: BlockType = "paragraph"
) {
  ...
  if (isEnd) {
    editor.insertBlock(targetBlockType);
  } else if (isStart) {
    editor.insertBlock(targetBlockType, { reverse: true });
  } else {
